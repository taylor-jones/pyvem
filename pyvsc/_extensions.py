...
from pyvsc._machine import machine_query
...
class GithubExtension(Extension):
    ...
    def _from_latest(self):
        ...
        if not self.prerelease:
            query_endpoint = '/repos/%s/%s/releases/%s' % (
                self.namespace, self.repo, self.release)
            ...
            query_endpoint = '/repos/%s/%s/releases' % (
                self.namespace, self.repo, self.release)
            query_url = '%s%s' % (_GITHUB_API_ROOT_URI, query_endpoint)
    ...
    def _from_args(self):
        ...
        endpoint = '/repos/%s/%s/releases/download/%s/%s' % (
            self.namespace, self.repo, self.release, self.asset_name)
