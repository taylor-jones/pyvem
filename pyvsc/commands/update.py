...
_HELP = Help(
    ...
    synopsis='{prog} update\n'
             ...
             ''.format(prog=_PROG),
    description='This command will update all extensions listed to their '
                'latest versions. It will also install missing extensions, if '
                'the extension(s) given to the `update` command are not yet '
                'installed.'
                '\n\n'
                'If no arguments are provided to the `update` command, '
                '`{prog}`' 'will default to updating all extensions for the '
                ...
                '\n\n'
                'By default, {prog} will also look for an update to the code '
                'editor. In order to bypass this check, the '
                '[example]--no-editor[/] option can be provided.'
                '\n\n'
                'In order to check for update to all of the installed code '
                'editor on the local system as well as all of their '
                'extensions, use:\n'
                '\t[example]{prog} update *[/]'.format(prog=_PROG)
)
